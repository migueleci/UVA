  ...
  #define MAX_N 700                         // second approach: O(n log n)
  ...
  int main() {
    ...
    while(gets(T)){
      ...
      //printf("%d\n", ans.first);
      bool eq,fp=true;
      if(ans.first==0) printf("No common sequence.\n");
      for(int i=0;i<n-1 && ans.first>0;i++){
        if(LCP[i+1]==ans.first){
